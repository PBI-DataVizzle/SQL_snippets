...
-- EXEC [USP].[SP_exec_create_dim_calendar_refresh]
AS
BEGIN
    SET NOCOUNT ON;
	SET DATEFIRST 1;
    ...
    BEGIN
		...
		-- DROP TABLE [PBI_DataStore].PBI.dim_calendar;
    END;
...
, additional_date_fields AS(
	SELECT
		...
		, CASE 
			  WHEN (DATEPART(WEEKDAY, date_id) + @@DATEFIRST - 1) % 7 NOT IN (6, 0) 
			  THEN 1 ELSE 0 
		  END AS is_weekday
		, CASE 
			  WHEN (DATEPART(WEEKDAY, date_id) + @@DATEFIRST - 1) % 7 IN (6, 0) 
			  THEN 1 ELSE 0 
		  END AS is_weekend
		...
		-- Corrected week start and end calculations
		, DATEADD(DAY, -(DATEPART(WEEKDAY, date_id) - 1), date_id) AS week_start
		, DATEADD(DAY, 6, DATEADD(DAY, -(DATEPART(WEEKDAY, date_id) - 1), date_id)) AS week_end

		-- Corrected week offset calculation
		, DATEDIFF(WEEK, 
			DATEADD(DAY, -(DATEPART(WEEKDAY, @current_date) - 1), @current_date), 
			DATEADD(DAY, -(DATEPART(WEEKDAY, date_id) - 1), date_id)
		) AS week_offset
...
SELECT 
	...
	, is_weekday
	, is_weekend
	, COALESCE( hol.is_holiday , 0 ) as is_holiday
	,	CASE
			WHEN is_weekday = 1
			AND is_weekend = 0
			AND COALESCE( hol.is_holiday , 0 )  = 0
		THEN 1 ELSE 0
		END AS is_workday
	, hol.holiday_name
	, quarter_num
...
--INTO [PBI_DataStore].PBI.dim_calendar
FROM additional_date_fields
LEFT JOIN ENT.sql_holidays as hol ON date_id = hol.holiday_date
)
INSERT INTO PBI.dim_calendar (
...
-- EXEC USP.[SP_exec_create_dim_calendar]
-- SELECT * FROM [PBI_DataStore].PBI.dim_calendar WHERE month_offset = 12
